...
static void _uriPlayerCallback(SLPlayItf uriP, void *context, SLuint32 event)
{
}

// create URI audio player
int  sl_uriPlayer_create(char *uri)
{
  //  SL_URI_PLAYER_t *uriPlayerP = &uriPlayerGroup[0];
    SLresult result;
	...
	*/
	...
	// register callback on the buffer queue
	result = (*uriPlayerPlay)->RegisterCallback(uriPlayerPlay, _uriPlayerCallback, NULL);
...
// set the playing state for the URI audio player
// to PLAYING (true) or PAUSED (false)
void sl_uriPlayer_set_play(int isPlaying)
{
    SLresult result;
...
{	
	...
	sl_uriPlayer_destroy();
	sl_uriPlayer_create(file);
	sl_uriPlayer_set_play(SL_PLAYSTATE_PLAYING);
}

#define sl_FILE_WAV1 "/sdcard/sound/no_answer.wav"
...
void audio_player_test()
{
	...
	while(1)
	{
		...
		usleep((2880+ i) * 1000);
		for(j = 0; j < i * 1000; j++);
		audio_player_play(sl_FILE_WAV2);
		usleep(300 * 1000);
		if (i == 200)
			i = 0;
		i += 5;
